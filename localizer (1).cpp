...
/**
    ...
    @return - a normalized two dimensional grid of floats. For 
           ...
           0.25 0.25
*/
//define function taking in 2D char vector and returning 2D float vector
vector< vector <float> > initialize_beliefs(vector< vector <char> > grid){
	vector< vector <float> > newGrid;                   //declares 2D vector
    int height = grid.size();                           //get # of rows
    //cout<<"IB "<<"height "<<height;
  	int width = grid[0].size();                         //get # of columns
    int area = height * width;
  	newGrid = zeros(height,width);
    float beliefPerCell = 1.0 / area;                   //create 2D vector representing normalized uniform distribution
    for (int i = 0; i < height; i++) {
        for (int j = 0; j < width; j++) {
            newGrid[i][j] = beliefPerCell;
        }
    }
...
vector< vector <float> > move(int dy, int dx, 
  ...
  float blurring) 
{
  vector < vector <float> > newGrid;                   //move this and assign zeros
  int height = beliefs.size();                           //get # of rows
  int width = beliefs[0].size();                         //get # of columns
  newGrid = zeros(height,width);
  
  //you need to go back and study the move stuff
  for (int i = 0; i < height; i++) {
      for (int j = 0; j < width; j++) {
          //newGrid[abs((i + dy) % height)][abs((j + dx) % width)] = beliefs[i][j];
          newGrid[(((i + dy)%height)+height)%height][(((j + dx)%width)+width)%width] = beliefs[i][j];
      }
  }
  // your code here
